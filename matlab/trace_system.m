function [ABCD,Px,Py] = trace_system(surface,nrays,hx,hy,dPx,dPy,fnex)
...
for abc = 1:5
    ...
    [xData1,yData1,zData1,lData1,mData1,nData1,~,~,~,~,~,Px,Py ] = MATLAB_BatchRayTrace_ReadRayData(...
...
for def = 1:5
    ...
    [xData2,yData2,zData2,lData2,mData2,nData2,~,~,~,~,~,Px,Py ] = MATLAB_BatchRayTrace_ReadRayData(...
            surface(2),nrays,hx(def),hy(def),dPx(def),dPy(def),fnex);
...
% Compute differential
    for ghi = 1:length(ray0in)
        ABCD{end+1} = compute_differential(ray0in(:,ghi),ray0out(:,ghi),...
